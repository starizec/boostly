            ...
            <div class="row flex-grow-1">
                ...
                <div class="col-md-4 grid-margin stretch-card">
                    <div class="card">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-baseline">
                                <h6 class="card-title mb-0">Chat Klikovi</h6>
                            </div>
                            <div class="row">
                                <div class="col-6 col-md-12 col-xl-5">
                                    @php
                                        $chatClickedStats = $eventStats['chat_clicked'] ?? ['all_time' => 0, 'mom_change' => null];
                                        $chatClickedMom = $chatClickedStats['mom_change'];
                                        $chatClickedUp = is_null($chatClickedMom) ? true : ($chatClickedMom >= 0);
                                        $chatClickedPercent = is_null($chatClickedMom) ? 0 : $chatClickedMom;
                                    @endphp
                                    <h3 class="mb-2">{{ number_format($chatClickedStats['all_time'] ?? 0) }}</h3>
                                    <div class="d-flex align-items-baseline">
                                        <p class="{{ $chatClickedUp ? 'text-success' : 'text-danger' }}">
                                            <span>{{ $chatClickedUp ? '+' : '' }}{{ number_format($chatClickedPercent, 1) }}%</span>
                                            <i data-lucide="{{ $chatClickedUp ? 'arrow-up' : 'arrow-down' }}" class="icon-sm mb-1"></i>
                                        </p>
                                    </div>
                                </div>
                                <div class="col-6 col-md-12 col-xl-7">
                                    <div id="chatKlikoviChart" class="mt-md-3 mt-xl-0"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-4 grid-margin stretch-card">
                    <div class="card">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-baseline">
                                <h6 class="card-title mb-0">Chat Startovi</h6>
                            </div>
                            <div class="row">
                                <div class="col-6 col-md-12 col-xl-5">
                                    @php
                                        $chatStartedStats = $eventStats['chat_started'] ?? ['all_time' => 0, 'mom_change' => null];
                                        $chatStartedMom = $chatStartedStats['mom_change'];
                                        $chatStartedUp = is_null($chatStartedMom) ? true : ($chatStartedMom >= 0);
                                        $chatStartedPercent = is_null($chatStartedMom) ? 0 : $chatStartedMom;
                                    @endphp
                                    <h3 class="mb-2">{{ number_format($chatStartedStats['all_time'] ?? 0) }}</h3>
                                    <div class="d-flex align-items-baseline">
                                        <p class="{{ $chatStartedUp ? 'text-success' : 'text-success' }}">
                                            <span>{{ $chatStartedUp ? '+' : '' }}{{ number_format($chatStartedPercent, 1) }}%</span>
                                            <i data-lucide="{{ $chatStartedUp ? 'arrow-up' : 'arrow-down' }}" class="icon-sm mb-1"></i>
                                        </p>
                                    </div>
                                </div>
                                <div class="col-6 col-md-12 col-xl-7">
                                    <div id="chatStartoviChart" class="mt-md-3 mt-xl-0"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-4 grid-margin stretch-card">
                    <div class="card">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-baseline">
                                <h6 class="card-title mb-0">Konverzije</h6>
                            </div>
                            <div class="row">
                                <div class="col-6 col-md-12 col-xl-5">
                                    @php
                                        $conversionStats = $eventStats['conversion'] ?? ['all_time' => 0, 'mom_change' => null];
                                        $conversionMom = $conversionStats['mom_change'];
                                        $conversionUp = is_null($conversionMom) ? true : ($conversionMom >= 0);
                                        $conversionPercent = is_null($conversionMom) ? 0 : $conversionMom;
                                    @endphp
                                    <h3 class="mb-2">{{ number_format($conversionStats['all_time'] ?? 0) }}</h3>
                                    <div class="d-flex align-items-baseline">
                                        <p class="{{ $conversionUp ? 'text-success' : 'text-danger' }}">
                                            <span>{{ $conversionUp ? '+' : '' }}{{ number_format($conversionPercent, 1) }}%</span>
                                            <i data-lucide="{{ $conversionUp ? 'arrow-up' : 'arrow-down' }}" class="icon-sm mb-1"></i>
                                        </p>
                                    </div>
                                </div>
                                <div class="col-6 col-md-12 col-xl-7">
                                    <div id="konverzijeChart" class="mt-md-3 mt-xl-0"></div>
                                </div>
                            </div>
                        </div>
                    </div>
...
@push('custom-scripts')
    ...
    <script>
        (function() {
            const el = document.querySelector('#chatKlikoviChart');
            if (!el || typeof ApexCharts === 'undefined') return;

            const categories = @json($chartCategories ?? []);
            const series = @json($chartSeriesChatClicked ?? []);
    ...
    <script>
        (function() {
            const el = document.querySelector('#chatStartoviChart');
            if (!el || typeof ApexCharts === 'undefined') return;

            const categories = @json($chartCategories ?? []);
            const series = @json($chartSeriesChatStarted ?? []);
    ...
    <script>
        (function() {
            const el = document.querySelector('#konverzijeChart');
            if (!el || typeof ApexCharts === 'undefined') return;

            const categories = @json($chartCategories ?? []);
            const series = @json($chartSeriesConversion ?? []);
